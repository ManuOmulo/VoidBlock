...
import com.voidblock.app.data.database.entities.BlockedAppEntity
...
class ScheduleChannel(private val context: Context) : MethodChannel.MethodCallHandler {
    ...
    companion object {
        const val CHANNEL_NAME = "com.voidblock.app/schedule"
    }
    ...
    private fun updateSchedule(data: Map<String, Any>?, result: MethodChannel.Result) {
        ...
        scope.launch {
            try {
                ...
                val stopIntent = android.content.Intent(context, com.voidblock.app.services.BlockingService::class.java).apply {
                    action = com.voidblock.app.services.BlockingService.ACTION_STOP_SCHEDULE
                    val blockedApps = database.blockedAppDao().getBlockedAppsForScheduleSync(schedule.id)
                        .map { it.packageName }
                    putStringArrayListExtra(com.voidblock.app.services.BlockingService.EXTRA_APP_PACKAGES, ArrayList(blockedApps))
                }
